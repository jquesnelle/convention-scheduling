...
\subsection{Problem descriptions and classes}
...
\begin{quote}
	...
	\begin{enumerate}[(c)]
		\item $\sum\limits_{i=1}^n f(i,j,h) = \sum\limits_{i=1}^nG_{ij}$ for all $1 \le j \le m$ and $h \in H$
	\end{enumerate}
\end{quote}
\begin{thm}
CTTD is NP-Complete.
\end{thm}
\begin{proof}
Suppose $S$ is a polynomial time solver of CTTD and let $L$ be an instance of TTD/CTTD. Consider the following algorithm.
\begin{algorithm}
\caption{A polynomial time solver of TTD}\label{ttd_poly}
\begin{algorithmic}[1]
	\State
\end{algorithmic}
\end{algorithm}
