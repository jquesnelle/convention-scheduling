...
%opening
\title{Integer programming models of certain NP-Complete convention scheduling problems}
...
In practice, the reduction of our solution space to only $\mathcal F'$ gives us an enormous performance gain. But, we run into an issue when trying to enforce requirement (c) (that all presenters are present for all instances of a talk). \textit{Co-presenters} are pairs of presenters that give the same talk. If co-presenters have different availability sets (given by $P_i \subseteq H$) then we cannot sufficiently enforce this in our programming model with only $\mathcal F'$. For example, if co-presenters $i,i'$ have availability sets $\{h_2,h_3\}$ and $\{h_3,h_4\}$ then $f_{i,j,h_4,r} \notin \mathcal F'$ for all $r \in S_j$, and likewise $f_{i',j,h_2,r} \notin \mathcal F'$ for all $r \in S_j$. We create a second set of variables $\mathcal F''$ with all of these ``extra" variables created by co-presenters with non-identical availability sets. Precisely, $f_{i,j,h,r}, f_{i',j,h',r} \in \mathcal F'' \iff$ $i$ and $i'$ are co-presenters of talk $j$, $h \in P_{i'}$, $h \notin P_{i}$, $h' \in P_i$, and $h' \notin P_{i'}$. We note that $\mathcal F' \cap \mathcal F'' = \emptyset$. With $\mathcal F'$ and $\mathcal F''$ defined, we have our final set of solution variables: $\mathcal F = \mathcal F' \cup \mathcal F''$. In addition to $\mathcal F$ we will use the intermediate variable set $\mathcal G$ for convenience where $g_{j,h,r} \in \mathcal G \iff $ talk $j$ can be given by any presenter at hour $h$ in room $r$.
...
&\sum_{i,j,h,r \in \mathcal F''} f_{i,j,h,r} = 0\label{ectto_ip_constraint_c_1}&\\
...
&\left(\sum_{i \in \mathcal F_{j,h,r}} f_{i,j,h,r}\right) - U \times g_{j,h,r} \le 0& \text{for each $j,h,r \in \mathcal G$} \label{ectto_ip_constraint_e_1}\\
...
We previously noted that requirement (a) is implicitly enforced by only considering those variables $\mathcal F'$ which represent valid schedules. This is not entirely correct. In our description of $\mathcal F'$ we gave an example of possible invalid entries in $\mathcal F'$ when co-presenters have different availabilities. We constructed $F''$ to ``fill out" $\mathcal F$ by adding $f_{i,j,h,r}$ for $h,r$ that is invalid for presenter $i$ and talk $j$, but \emph{are} valid for co-presenters of talk $j$. This ensures that for co-presenters $i, i'$ of talk $j$ there exists the same set of $h,r$ variables for them. To ensure requirement (c) we force that all co-presenters have the same schedule for their common talk (constraint \ref{ectto_ip_constraint_c_2}) but that the invalid ones must be zero (constraint \ref{ectto_ip_constraint_c_1}).
