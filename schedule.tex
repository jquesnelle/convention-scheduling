...
&\left(\sum_{j,h \in \mathcal G_{r}} g_{j,h,r}\right) - U \times z_{j,h} \le 0& \text{for each room $r$}\\
&\sum_{h \in \mathcal Z_{j}} z_{j,h} = G_{ij}& \text{for each talk $j$ \text{ and some presenter $i$}}\\
&z_{j,h} + z_{j',h} - 2 \times z'_{j,j',h} = 1& \text{for each hour $h$ and $j,j' \in \mathcal Z_h$ with $j \ne j'$} \\
&c_{j,j',h} - w_{j,j',h} \times z'_{j,j',h}& \text{for all $j,j',h \in \mathcal {Z'}$}\\
